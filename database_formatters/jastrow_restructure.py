...
def strip_nikkud(token):
    ...
    """
    alph_only = ''.join([c for c in token if not (1456 <= ord(c) <= 1479) or c == '־'])
    return alph_only
...
if __name__ == '__main__':
    ...
    for d in docs:
        ...
        new_forms = [order_nikkud( word_only(w) ) for w in new_forms]
        ...
        # (4)
        ordered_pl = [order_nikkud( word_only(w) ) for w in d['plural_form']]
        ordered_alt = [order_nikkud( word_only(w) ) for w in d['alt_headwords']]
